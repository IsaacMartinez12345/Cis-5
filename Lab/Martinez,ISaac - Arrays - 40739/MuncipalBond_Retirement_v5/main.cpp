...
//Function Prototypes
void savings(vector<float>&,vector<int>&,vector<int>&,float &,int,float,float,float);
void display(vector<float>&,vector<int>&,vector<int>&,float,int,float,float,float);
...
int main(int argc, char** argv) {
    ...
    float interest;       //Interest Calculation    //Year date  Parallel Array
    float yrDep;
    float sav2Rtr;
    ...
    savings(balance,year,dYear,intRate,NYEARS,yrDep,yrDpPc,salary);
    ...
    display(balance,year,dYear,intRate,NYEARS,yrDep,salary,yrDpPc);
...
void display(vector<float> &balance,vector<int> &year,vector<int> &dYear,float intRate,int NYEARS,float yrDep,float salary,float yrDpPc){
...
void savings(vector<float> &balance,vector<int> &year,vector<int> &dYear,float &intRate,int NYEARS,float yrDep,float yrDpPc,float salary){
    //Process or map Inputs to Outputs
    intRate/=CNVDCML;
